	...
	public String parseFunction(List<String> list, IWikiModel model, char[] src, int beginIndex, int endIndex, boolean isSubst)
		...
		if (list.size() > 0) {
			...
			if (arg0.length() > 0 && list.size() == 1) {
				String result = "/wiki/" + URLEncoder.encode(Character.toUpperCase(arg0.charAt(0)) + "", "UTF-8")
						+ URLEncoder.encode(arg0.substring(1), "UTF-8");
				return result;
			}
			StringBuilder builder = new StringBuilder(arg0.length() + 32);
			builder.append("/w/index.php?title=");
			builder.append(URLEncoder.encode(Character.toUpperCase(arg0.charAt(0)) + "", "UTF-8"));
			builder.append(URLEncoder.encode(arg0.substring(1), "UTF-8"));
